...
{
    /// <summary>
    /// A registry of the concrete types of <see cref="IValueFormatter"/> which are available for use by the Screenplay reporting functionality.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// This is the mechanism which is used as a tie-breaker if two or more <see cref="IValueFormatter"/> types would both return <see langword="true" />
    /// from <see cref="IValueFormatter.CanFormat(object)"/>.
    /// </para>
    /// <para>
    /// Developers might want to add a <see cref="IValueFormatter"/> type to this collection, which is to have a lower precedence than another which has already
