...
public class StepDefinitionsWithDependencies(
    ...
    FeatureScopeService<IFeatureDependency> featureDependency,
    WorkerScopeService<IWorkerDependency> workerDependency,
    ITestRunDependency testRunDependency)
{
    ...
    {
        outputHelper.WriteLine($"Feature dependency used {featureDependency.Instance.Use()} times");
    }

    [When("a worker dependency is used")]
    public void WhenAWorkerDependencyIsUsed()
    {
        outputHelper.WriteLine($"Worker dependency used {workerDependency.Instance.Use()} times");
    }
