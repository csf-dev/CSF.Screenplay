...
namespace CSF.Screenplay.Selenium.Tests.Actions
{
  ...
  public class TargetNotFoundTests
  {
    ...
    public void Click_on_non_existent_element_raises_TargetNotFoundException(ICast cast, Func<BrowseTheWeb> webBrowserFactory)
    {
    ...
    public void TargetNotFoundException_raised_has_correct_target_name(ICast cast, Func<BrowseTheWeb> webBrowserFactory)
    {
      var joe = cast.GetJoe(webBrowserFactory);
    ...
    public void TargetNotFoundException_includes_target_name_in_report(ICast cast, Func<BrowseTheWeb> webBrowserFactory)
    {
      var joe = cast.GetJoe(webBrowserFactory);
    ...
    [Test,Screenplay]
    [Description("A 'target not found' exception should include the target name in its report when the target is provided.")]
    public void TargetNotFoundException_can_create_a_report_without_target(ICast cast, Func<BrowseTheWeb> webBrowserFactory)
    {
      var joe = cast.GetJoe(webBrowserFactory);
